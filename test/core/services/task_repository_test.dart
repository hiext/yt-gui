...
void main() {
  ...
  group('TaskRepository', () {
    ...
    test(
      'loadPendingTasks returns queued, downloading, paused tasks',
      () async {
        final pending = [
          DownloadTask(
            id: 'q-1',
        ...
        expect(loaded.map((t) => t.status).toSet(), {
    ...
    test(
      'savePendingTasks replaces pending statuses with given tasks',
      () async {
    ...
    test('loadHistoryTasks returns completed, failed, cancelled', () async {
      ...
      expect(history.map((t) => t.status).toSet(), {
