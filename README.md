...
- 换声（人声替换）：离线本地处理，保留背景音乐，内置多款中文预设备色
...
换声功能在本地完成「人声/伴奏分离 → 语音识别分句 → 逐句合成新音色 → 混回伴奏」的全流程，视频中的背景音乐会保留，无需联网调用云端。

- **离线处理**：识别、合成、分离全部在本机运行；模型在首次使用时下载（约 325MB），之后可离线使用。
- **预设备色**：内置 8 款 Kokoro 中文预设备色（4 女声 + 4 男声），可在设置页切换默认音色。
- **模型目录**：默认位于用户数据目录（macOS：`~/Library/Application Support/hiext_yt_gui`，模型文件在其中的 `models/` 子目录），可在设置页自定义。
- **内置工具**：人声分离使用随包发布的 `sherpa-onnx` 分离 CLI（macOS / Linux / Windows），与 `yt-dlp`/`ffmpeg` 使用同一套解析规则。
- **许可提示**：模型权重遵循各自许可（UVR 需保留署名、SenseVoice 需保留模型名、Kokoro 为 Apache-2.0），详见 [第三方工具声明](docs/third-party-tools.md)。

> 换声结果是机器合成语音，请确认使用场景与内容权限。实时变声（RVC 等）已列入后续规划，本期不提供。
...
- [换声功能设计](docs/codex-voice-swap/voice-swap-design.md)：换声模块的需求边界、技术选型、配置项与上线清单。
- [换声上线材料](docs/codex-voice-swap/release.md)：设置项 properties、模型 sha256 回填说明与分阶段验收记录。
